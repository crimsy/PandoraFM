...
<tr>
  <?php
$lastxmlurl = "http://ws.audioscrobbler.com/1.0/user/$username/recenttracks.xml";
$init = curl_init();
...
if (count($entries) == 0) {die("Sorry, there was an error."); }
$c = 0;
foreach ($entries->track as $trackname) {
	...
	$artistname = $trackname->artist;
	$track = substr($trackname->name,0,25);
	...
	#echo "<td width=48% class=line1>".'<a href=# TITLE="header=[Actions] body=[Tag, Love, Ban or Reccomend this track] delay=[800] fade=[off] fadespeed=[0.04] fixedrelx=[0] fixedrely=[0] cssheader=[popupheader] cssbody=[popupbody]">'."$artistname - $track</a></td>";
	echo "<td width=48% class=line1>"."$artistname - $track</a></td>";
