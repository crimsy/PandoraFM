...
if ($username && $password && !stristr($username,"@")) {
	...
	echo '<script>top.location.href="/?notvalid=1"</script>';
}
	...
	if ($handshake[0]=='OK') {
		...
		if ($exec == 'OK') {
			...
			if ($exec == "BADAUTH") {
				...
				echo '<script>top.location.href="http://pandorafm.real-ity.com/?notvalid=1"</script>';
				lfmlog("$timestamp,$username,$artist,$song,Invalid Login,$attempt");
			}
			
			if (strpos($exec,'invalid username')) {
				lfmlog("$timestamp,$username,$artist,$song,Invalid Login,$attempt");
				echo '<script>top.location.href="http://$servername/?notvalid=1"</script>';	
			}
